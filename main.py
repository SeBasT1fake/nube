...
from pyspark.sql.functions import col, explode, abs
import sys
...
try:
    ...
    weather_raw = spark.read.json("file.json")
    ...
    cities_df = spark.read.option("header", True).csv("cities.csv")
    ...
    alerts_df = spark.read.option("header", True).csv("weather_alerts.csv")
    ...
    print("✅ Escribiendo resultados a trusted")
    joined_data.write.mode("overwrite").parquet("weather_processed/")
